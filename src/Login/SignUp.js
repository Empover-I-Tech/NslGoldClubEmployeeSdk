...
import { View, Platform, StatusBar, Text, Image, Linking, Keyboard, TouchableOpacity, Modal, Dimensions, Alert, PermissionsAndroid, TextInput, StyleSheet } from 'react-native';
...
function SignUp({ route }) {
  ...
  useFocusEffect(
    React.useCallback(() => {
      console.log('useFocusEffect - SignUp', route?.params?.loaderPath)
          ...
          <View style={styleSheetStyles.modalView}>
            ...
            <CustomButton
              ...
              onPress={async () => {
                ...
                navigation.reset({ index: 0, routes: [{ name: (roleTypeDetails === 'Retailer' || roleTypeDetails === 'Distributor') ? 'RetailerDashboard' : 'EmployeeDashboard', params: {} }] })
                //  roleNameNav
              }}
  ...
  const onSelectedState = async (item, selectedItemFromGps, cleanedSubDistrict) => {
    ...
    if (item?.code.toLowerCase() == translate('all').toLowerCase()) {
      ...
      if (selectedItemFromGps && filterDistList?.length > 0) {
        ...
        const apiDistrict = normalize(selectedItemFromGps);

        const matchedDistrict = filterDistList.find(dist =>
          normalize(dist.name) === apiDistrict ||
          normalize(dist.name).includes(apiDistrict)
        );

        console.log('matchedDistrict', matchedDistrict);
        // ✅ Auto select only if match found
        if (matchedDistrict) {
          onSelectedDistrict(matchedDistrict, cleanedSubDistrict);
        }

        // ❌ If no match → nothing happens (existing flow continues)
      }
  ...
  const onSelectedDistrict = async (item, cleanedSubDistrict) => {
    ...
    if (item?.code.toLowerCase() == translate('all').toLowerCase()) {
      ...
      if (cleanedSubDistrict && filterSubDistList?.length > 0) {
        ...
        const apiSubDistrict = normalize(cleanedSubDistrict);

        const matchedSubDistrict = filterSubDistList.find(dist =>
          normalize(dist.name) === apiSubDistrict ||
          normalize(dist.name).includes(apiSubDistrict)
        );
      ...
      if (response.data && response.data.results) {
        ...
        if (
          ...
          const matchedState = stateList.find(item =>
            normalize(item.name) === apiState ||
            normalize(item.name).includes(apiState)
          );

          // ✅ Only if match found → update
          if (matchedState) {
            onSelectedState(matchedState, cleanedDistrict, cleanedSubDistrict);
          }

          // ❌ If no match → nothing happens (normal flow continues)
        }
