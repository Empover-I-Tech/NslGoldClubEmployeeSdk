...
function RetailerDashboard({ route }) {
    ...
    try {
      ...
      if (APIResponse?.statusCode === HTTP_OK) {
        ...
        if (latestPopup?.dailyOnce) {
          ...
          console.log("sasasasasa", savedDate)
  ...
  const [secondaryScanerVisible, setSecondaryScanerVisible] = useState(false)
  ...
  const getVillageMasterCall = async (selectedVillageCode) => {
    if (networkStatus) {
      try {
        ...
        console.error('Village Master Fetch Error:', error.message);
      }
      finally {
        stopLoading();
      }
  ...
  useEffect(() => {
    ...
    const callAutoFetchVillage = async () => {
      if (autoFetchProfileDetails?.villageName && villageListOriginal?.length > 0) {

        const matchedVillage = await findMatchedItem(villageListOriginal, autoFetchProfileDetails?.villageName, 'name');
  ...
  useEffect(() => {
    ...
    if (
      ...
      return; // ⛔ Stop until everything is ready
    }

    const callAutoFetchState = async () => {

      const matchedState = await findMatchedItem(
        stateList,
        autoFetchProfileDetails.stateName,
        'name'
      );
      ...
      if (matchedState) {
        await onSelectedState(
          matchedState,
          autoFetchProfileDetails.districtName,
          autoFetchProfileDetails.subDistrictName
        );
      }
    };
  ...
  }, [autoFetchProfileDetails, stateList, districtListOriginal, subDistrictListOriginal]);


  useEffect(() => {
    if (showDetailViewModal === true) {
      if (subDistrictID != '' && subDistrict != '' && networkStatus) {
        getVillageMasterCall(subDistrictID)
      }
    }
  }, [subDistrictID, showDetailViewModal])
  ...
  const GetPointsOnly = useCallback(async alertSync => {
    ...
    try {
      ...
      if (APIResponse?.statusCode === HTTP_OK) {
        ...
        setShowProfilePopup(APIResponse?.response?.userList[0]?.isProfileUpdated)
        setProfileMessage(APIResponse?.response?.userList[0]?.profileMessage)
        ...
        console.log("dashboardResp?.secondaryScanerVisible111", dashboardResp?.secondaryScanerVisible)
        setSecondaryScanerVisible(dashboardResp?.secondaryScanerVisible || false)
      ...
      console.error('EmployeeDashboard Fetch Error:', error.message);
      setSuccessLoadingMessage(error.message);
    } finally {
      setPointsLoader(false);
    }
  ...
  const checkDataFromRealm = async () => {
    ...
    setTimeout(async () => {
      ...
      if (scannedCoupons.length > 0 && networkStatus && couponsSyncing == false) {
        ...
        if (!calledOnce) {
          if (networkStatus) {
            startLoading(translate('syncingPlz'))
          }
    ...
    if (!networkStatus) {
      ...
      if (pointsData) {
        ...
        setSecondaryScanerVisible(pointsData?.secondaryScanerVisible || false)
        ...
        setSecondaryScanerVisible(dashboardData?.secondaryScanerVisible || false)
      }
    } else {
  ...
  const navigateToPropelSdk = useCallback(async () => {

    var networkStatus = await getNetworkStatus()
    if (networkStatus) {
      var getHeaders = await GetApiHeaders()

      // const AUTH_TOKEN = '8540f0e5-029c-4a80-bbc8-7da952e1b1d3:7630a8c22d0c9dbcbf1b86b03bd58bf2';
      // const AUTH_ID = 'NUZIVEEDUS';
      // const API_URL = 'https://staging-propel.zaggle.in';    
      // const CLIENT_NAME = 'AndroidApp';
      // var userId = await getUserIdFromStorage();
      // var deviceId = await getDeviceId();
      // const sdkVersion = require('@propel-nsl/propel-react-native-sdk/package.json').version;
      ...
      //   authId: AUTH_ID,
      //   apiUrl: API_URL,
      //   environment: !APP_ENV_PROD ? 'staging' : 'production',
      //   deviceId: deviceId,
      //   clientName: Platform.OS == 'android' ? CLIENT_NAME : Platform.OS,
      ...
      try {
        startLoading(translate('please_wait_getting_data'))
        var propelUrl = configs.BASE_URL + configs.REDEEM.PROPELSDK_API;
        var getHeaders = await GetApiHeaders()
        const sdkVersion = require('@propel-nsl/propel-react-native-sdk/package.json').version;
        ...
        console.log('propelUrl is', propelUrl)
        console.log('getHeaders is', getHeaders)
        var APIResponse = await PostRequest(propelUrl, getHeaders, requestBody);
        if (APIResponse != undefined && APIResponse != null) {
          console.log("APIResponsewrrere", APIResponse)
          if (APIResponse.status && APIResponse.session != null) {
            var responsesdk = APIResponse.session
            console.log('the responsesdk resp is', responsesdk)
            if (responsesdk != undefined && responsesdk != null) {

              navigation.navigate('PropelSDK', {

                screen: responsesdk?.screenName,
                authToken: responsesdk?.authToken,
                authId: responsesdk?.authId,
                apiUrl: responsesdk?.apiUrl,
                environment: responsesdk?.environment,
                clientName: responsesdk?.clientName,
                clientVersion: sdkVersion,
                deviceId: getHeaders?.deviceId,
                userId: responsesdk?.employeeId,
                clientId: responsesdk?.clientId
              })
              setShowCustomActionSheet(false)
            }
          }
  ...
  const renderProgramGroup = (group) => {
    return (
  ...
  const Card = ({ item, index }) => {
    return (
      <TouchableOpacity
        disabled={true}
        style={[styles['alignItems_center'], styles[''], styles['border_width_1'],
        { borderColor: item?.primaryColor }, styles['border_radius_10'], styles['margin_top_10'], styles['width_100%']]}>

        <View style={[styles['flex_direction_row'], { alignItems: "center", width: "100%", paddingHorizontal: responsiveWidth(1), paddingVertical: responsiveHeight(1) }]}>
          ...
          <View style={[styles['left_5'], styles['align_self_center']]}>
            ...
            <View style={[styles['flex_direction_column'], styles['margin_top_3'], { width: '90%' }]}>

              <View style={[styles['flex_direction_row']]}>
                <Text
                  allowFontScaling={false}
                  style={[
                    { color: dynamicStyles.textColor, width: '45%' }, // fixed width for alignment
                ...
                <Text
                  allowFontScaling={false}
                  style={[{ color: dynamicStyles.textColor, width: '45%' }, styles['font_size_10_bold']]}
                >
                  {item.companyName || ''}
                </Text>
              </View>
              ...
              <View style={[styles['flex_direction_row']]}>
                <Text
                  allowFontScaling={false}
                  style={[
                    { color: dynamicStyles.textColor, width: '45%' }, // same width as above
                    styles['font_size_10_regular']
                  ]}
                >
    ...
    return (
      <Modal animationType="slide"
        ...
        <View style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: Colors.blackTransparent }}>
          ...
          <TouchableOpacity onPress={() => resetSelectedItems()}>
            <Image style={[styles['width_100%'], styles['height_60'], { position: 'relative', top: 20 }]} resizeMode='stretch' source={require('../assets/images/pyramid.png')}></Image>
            <Image style={[{ width: 25, height: 25, position: 'absolute', right: 20, top: 45, tintColor: dynamicStyles?.primaryColor }]} resizeMode='stretch' source={require('../assets/images/close.png')}></Image>
          </TouchableOpacity>
          <View style={[{ overflow: 'hidden', padding: 15, backgroundColor: "white", height: responsiveHeight(60), marginTop: 10 }]}>
          ...
          <View style={{ backgroundColor: "white", width: '100%', bottom: 10 }}>
            <CustomButton
              ...
              onPress={() => { (showQuickProceed && couponScanHistoryList.length > 0) ? navigateToPropelSdk() : SimpleToast.show(translate('you_dont_have_sufficient_balance')) }}
  ...
  const handleCancelAlert = () => {
    ...
    if (alertMessage === "Successfully loaded amount in to the card") {
      GetRedeemPointsList()
      showQuickProceed && navigateToPropelSdk()
      ...
      try {
        ...
        if (APIResponse != undefined && APIResponse != null) {
          ...
          if (APIResponse.statusCode == HTTP_OK) {
            ...
            const totalAvailablePoints = APIResponse?.response?.totalPointsAvailble || 0;
            ...
            // const groupedList = groupByProgramName(rewardsPoints?.couponScanHSummaryList);
            const groupedList = await groupByProgramName(list);
            // kiran 
            setCouponScanHistoryList(groupedList)
          }
          else {
            setTimeout(() => {
  ...
  const submitApiCall = async (userDataForm) => {
    ...
    if (networkStatus) {
      try {
        startLoading(translate('submitting_data'))
        ...
        var jsonData = {
          ...
          "subDistrictId": subDistrictID != undefined ? subDistrictID : "",
          "subDistrictName": subDistrict != undefined ? subDistrict : "",
          "villageId": villageID != undefined ? villageID : "",
          "villageName": village != undefined ? village : "",
  ...
  const rasiseRequestAPICall = async () => {
    ...
    if (networkStatus) {
      try {
        startLoading(translate('please_wait_mpin_verifing'))
  ...
  const GetDashboardDetailsApiCall = useCallback(async (alertSync) => {
    ...
    try {
      ...
      if (APIResponse?.statusCode === HTTP_OK) {
        ...
        setUserPointsEarned(dashboardResp?.userPointsEarned);
        console.log("dashboardResp?.secondaryScanerVisible222", dashboardResp?.secondaryScanerVisible)
        setSecondaryScanerVisible(dashboardResp?.secondaryScanerVisible || false)
        setUserPointsReedemed(dashboardResp?.userPointsReedemed);
        setTotalRetailers(dashboardResp?.totalRetailers);
        setActiveRetailers(dashboardResp?.activeRetailers);
  ...
  const clearDashboardRealmData = async () => {
    try {
      // setHomePageData([])
      realm.write(async () => {
  ...
  const onSelectedState = async (item, selectedItemFromGps, cleanedSubDistrict) => {
    ...
    if (item?.code.toLowerCase() == translate('all').toLowerCase()) {
      ...
      if (selectedItemFromGps && filterDistList?.length > 0) {
        ...
        if (matchedDistrict) {
          await onSelectedDistrict(matchedDistrict, cleanedSubDistrict);
        }
      }
    ...
    if (item?.code.toLowerCase() == translate('all').toLowerCase()) {
      ...
      if (cleanedSubDistrict && filterSubDistList?.length > 0) {
        ...
        if (matchedSubDistrict) {
          await onSelectedSubDistrict(matchedSubDistrict);
        }
      }
  ...
  const onPressCarouselButton = (item) => {
    ...
    if (item?.navigationTo != null && item?.navigationTo != undefined, item?.navigationTo?.length > 0) {
      ...
      //  setMediaLink("https://www.aeee.in/wp-content/uploads/2020/08/Sample-pdf.pdf")
      //  setMediaLink("https://youtu.be/yeRtHMvnNHE?si=GG6JADWX-N-Jnb3x")
      if (item?.urlPath) {
  ...
  const getUserMenUCOntrollerUI = () => {
    return (
      <View style={{
        ...
        paddingBottom: (weatherIsVisible && weatherInfo) ? 100 : 20,
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.5} style={[styles['flex_direction_row'], styles['width_48%'], styles['height_65'], styles['bg_white'], styles['border_radius_6'], styles['centerItems']]} onPress={() => {
            ...
            if (networkStatus) {
              ...
              console.log('ssssssssssssssssssssss11', showRedeemKyc);
              console.log('eeeeeeeeeeeeeeeeeeeeeee22', editKyc);
              if (showRedeemKyc) {
                setShowPanEntryModal(true)
              } else if (editKyc) {
                setShowEdit(true);
  ...
  const renderProfileUpdatePopUp = () => {
    return (
      <Modal
        transparent
        animationType='fade'
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View style={{
            ...
            <View style={[
              ...
              <Text style={[styles['font_size_14_regular'], styles['text_color_black'], styles['text_align_center']]}>{profileMessage || ''}</Text>
              ...
              <CustomButton
                ...
                onPress={() => {
                  ...
                  navigation.navigate('MyAccount', { badgeIcon: badgeIcon, comingFromDashboard: true });
                }}
                buttonBg={dynamicStyles.primaryColor}
                btnWidth={'90%'}
                titleTextColor={dynamicStyles.secondaryColor}
