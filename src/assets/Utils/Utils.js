...
export const POPUP_SHOWN_DATE = "POPUP_SHOWN_DATE";
...
export async function filterArrayOfObjects2(array, field1, code1, field2, code2) {
    console.warn("Filtering objects here:", array.length + "--" + field1 + "--" + code1+ "--" + field2 + "--" + code2);
...
export const checkIfGpsEnabled = async () => {
    if (Platform.OS === "android") {
        try {
            ...
            if (err?.code === "ERR00" || err?.message?.includes("cancelled")) {
                return false;
            }

            // ✅ Only show alert instead of auto redirect
            Alert.alert(
                "Enable GPS",
                "Please enable your device’s GPS",
                [
                    { text: "Cancel", style: "cancel" },
                    {
                        text: "Open Settings",
...
export const findMatchedItem = async (list = [], searchText = '', key = 'name') => {
    console.log("Finding matched item in list:", list.length, "searchText:", searchText, "key:", key);
    if (!Array.isArray(list) || !searchText?.trim()) return null;

    const normalizedSearch = normalizeText(searchText);

    return list.find(item => {
        const itemText = normalizeText(item?.[key] || '');

        return (
            itemText === normalizedSearch ||
            itemText.includes(normalizedSearch)
        );
    });
};

export const groupByProgramName = async (list) => {
    const result = {};

    list.forEach((item) => {
        if (!result[item.programName]) {
            result[item.programName] = [];
        }
        result[item.programName].push(item);
    });
...
export const traverseAndReplaceUrls = async (data) => {
